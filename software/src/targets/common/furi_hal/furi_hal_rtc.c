...
#define FURI_HAL_RTC_LSE_STARTUP_TIME LSE_STARTUP_TIMEOUT
...
static const FuriHalSerialId furi_hal_rtc_log_devices[] = {
    ...
    [FuriHalRtcLogDeviceLpuart] = FuriHalSerialIdMax,
...
static void furi_hal_rtc_start_clock_and_switch(void) {
    ...
    uint32_t timeout = FURI_HAL_RTC_LSE_STARTUP_TIME;
    while((LL_RCC_LSE_IsReady() != 1) && timeout) {
        LL_mDelay(1);
        timeout--;
    }

    if(LL_RCC_LSE_IsReady() != 1) {
        furi_crash("LSE failed");
    }
...
uint32_t furi_hal_rtc_get_register(FuriHalRtcRegister reg) {
    return LL_RTC_BKP_GetRegister(RTC, reg);
}

void furi_hal_rtc_set_register(FuriHalRtcRegister reg, uint32_t value) {
    LL_RTC_BKP_SetRegister(RTC, reg, value);
}
