...
#ifdef FURI_DEBUG
#define FURI_DEBUG_TASK_STATUS_COUNT 16U

static TaskStatus_t furi_debug_task_status[FURI_DEBUG_TASK_STATUS_COUNT];
static uint32_t furi_debug_task_total_run_time = 0;
static UBaseType_t furi_debug_task_status_count = 0;
volatile void* furi_debug_thread_helpers_keepalive[5] = {0};

TaskHandle_t furi_debug_get_task_handle(const char* name) {
    return xTaskGetHandle(name);
}

const char* furi_debug_get_task_name(TaskHandle_t task) {
    return pcTaskGetName(task);
}

UBaseType_t furi_debug_refresh_task_status(void) {
    UBaseType_t task_count = uxTaskGetNumberOfTasks();
    if(task_count > FURI_DEBUG_TASK_STATUS_COUNT) {
        task_count = FURI_DEBUG_TASK_STATUS_COUNT;
    }

    furi_debug_task_status_count =
        uxTaskGetSystemState(furi_debug_task_status, task_count, &furi_debug_task_total_run_time);

    return furi_debug_task_status_count;
}

TaskStatus_t* furi_debug_get_task_status(void) {
    return furi_debug_task_status;
}

UBaseType_t furi_debug_get_task_status_count(void) {
    return furi_debug_task_status_count;
}
#endif
...
#ifdef FURI_DEBUG
    furi_debug_thread_helpers_keepalive[0] = furi_debug_get_task_handle;
    furi_debug_thread_helpers_keepalive[1] = furi_debug_get_task_name;
    furi_debug_thread_helpers_keepalive[2] = furi_debug_refresh_task_status;
    furi_debug_thread_helpers_keepalive[3] = furi_debug_get_task_status;
    furi_debug_thread_helpers_keepalive[4] = furi_debug_get_task_status_count;
#endif
}
